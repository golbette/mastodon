...
class AuthorizeActionService < BaseService
  def call(source_account, target_account, **options, action)
    if options[:skip_follow_request]
      general_request = GeneralRequest.new(account: source_account, target_account: target_account, uri: options[:follow_request_uri], request_type: action)
    else
      general_request = GeneralRequest.find_by!(account: source_account, target_account: target_account, request_type: action)
      general_request.authorize!
    end
